...
player1_score = [["ones", -1], ["twos", -1], ["threes", -1],
                 ["fours", -1], ["fives", -1], ["sixes", -1]]
player2_score = [["ones", -1], ["twos", -1], ["threes", -1],
                 ["fours", -1], ["fives", -1], ["sixes", -1]]
...
def manage_player_hand(dices_on_board, player_hand):
    while True:
        ...
        if pick_discard_dice.isalpha():
            ...
            if pick_discard_dice == "p":
                ...
                while dices_on_board:
                    ...
                    print("There are no dices on board")
...
def select_points(board, player_hand, player_score):
    ...
    while True:
        while True:
            ...
            if not score_choice.isdigit():
                ...
                break
        if score_choice < 1 or score_choice > 6 or player_score[score_choice - 1][1] != -1:
            print("Choose between 1-6 or an available number!")
        elif (score_choice not in player_hand) and (score_choice not in board):
            ...
            if zero_points_choice.lower() == "y":
                player_score[score_choice - 1][1] = 0
                break
...
def main():
    ...
    for i in range(6):
      ...
      # player1 plays
        ...
        for roll in range(3):
            dice_kept = 5 - len(p1)
            board = roll_dices(dice_kept)
            manage_player_hand(board, p1)
        select_points(board, p1, player1_score)
        print_scores()
        ...
        for roll in range(3):
            dice_kept = 5 - len(p2)
            board = roll_dices(dice_kept)
            manage_player_hand(board, p2)
        select_points(board, p2, player2_score)
        print_scores()
